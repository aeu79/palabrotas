...
def carga_diccionario():
    ...
    with zipfile.ZipFile(zip_path, 'r') as myzip:
        ...
        with myzip.open(name) as file:
            ...
            set_dict = set(word.split('/')[0].strip() for word in lineas) #Armo un set (mucho más rápido porque usa hashes). Me quedo solo hasta el '/' y saco el espacio de las que no lo tienen.
...
def inicio(modo):
    ...
    if modo == "diccionario":
        ...
        letras = input('Ingrese las letras combinables y palabras para usar enteras después de una "/" (nada para terminar).\nLetras: ')
    if letras == '':
        quit()
...
def filtrar(candidatas, letras):
    ...
    '''
    filtrada = [palabra for palabra in candidatas if letras in palabra] # Toma sólo las que contienen las letras elegidas para filtrar. List comprehension with conditional if... Tomá mate!
    print(len(filtrada))
...
def palabrotas(modo, letras, longitud):
    ...
    if modo == "completo":
        ...
        if type(letras) is list:
            if len(letras) >5:
                ...
                new_long = str(longitud-len(letras[1])-len(letras[2])-len(letras[3])+3) #restando las letras de la palabra fija
                for x in letras[1]:
                    letras[0] = letras[0].replace(x,'',1) #borro del listado las que voy a usar
                for x in letras[2]:
                    letras[0] = letras[0].replace(x,'',1) #borro del listado las que voy a usar
                for x in letras[3]:
                    letras[0] = letras[0].replace(x,'',1) #borro del listado las que voy a usar
                letras[0] += '123'
                combinadas = [''.join(it).replace('1', letras[1]).replace('2', letras[2]).replace('3', letras[3]) for it in itertools.permutations(letras[0].join('123'), int(new_long)) if len(''.join(it).replace('1', letras[1]).replace('2', letras[2]).replace('3', letras[3])) == longitud]
                ...
                new_long = str(longitud-len(letras[1])-len(letras[2])+2) #restando las letras de la palabra fija
                for x in letras[1]:
                    letras[0] = letras[0].replace(x,'',1) #borro del listado las que voy a usar
                for x in letras[2]:
                    letras[0] = letras[0].replace(x,'',1) #borro del listado las que voy a usar
                letras[0] += '12'
                combinadas = [''.join(it).replace('1', letras[1]).replace('2', letras[2]) for it in itertools.permutations(letras[0].join('12'), int(new_long)) if len(''.join(it).replace('1', letras[1]).replace('2', letras[2])) == longitud]
            elif len(letras) == 2: # ['asdfasdf','perro']
                new_long = str(longitud-len(letras[1])+1) #restando las letras de la palabra fija
                for x in letras[1]:
                    letras[0] = letras[0].replace(x,'',1) #borro del listado las que voy a usar
                letras[0] += '1'
                combinadas = [''.join(it).replace('1', letras[1]) for it in itertools.permutations(letras[0], int(new_long)) if len(''.join(it).replace('1', letras[1])) == longitud]
            elif len(letras) == 1: # ['asdfasdf']
                combinadas = [''.join(it) for it in itertools.permutations(letras[0], longitud)]
            else: print('No debería llegar nunca a este punto')
        '''
        ...
        candidatas = set(word for word in combinadas if word in set_dict) # transformo en set para evitar repetidos: 'zafar', 'zafar' (dos "a" en el input y las considera distintas)
        print('Son ' + str(len(combinadas)) + ' posibilidades.' + 'Tardó ' + str(round(time.time()-t,3)) + ' segundos en combinarlas.')
    elif modo == 'diccionario':
        # Primero filtro por tamaño:
        combinadas = [palabra for palabra in set_dict if len(palabra)==longitud] # Traigo todas las del diccionario con el tamaño correcto.
        # Ahora las que tienen las letras correctas:
        combinables = letras[0] # Las letras que se deben usar
        letters = set(combinables) # Las letras que se deben usar, pero una sola vez cada letra, sin repetir.
        # Filtro solo las que tienen las letras del listado.
        candidatas = [palabra for palabra in combinadas if set(palabra).issubset(letters)]
        ...
        for palabra in candidatas:
            ...
            resultado = 'OK'
            for letra in letras:
                if palabra.count(letra) > frecuencias[letters.index(letra)]: #letters tiene el tuple de letras combinables únicas. Con index traigo su posicón para buscar en frecuencias el número de ocurrencias.
                    resultado = 'tiene más'
                    break
            if resultado == 'OK':
    ...
    if rta != '': #Uso las letras que ingresa para filtrar los resultados un poco más (y ganarle a Matías)
        filtrar(candidatas, rta)
    if modo != 'diccionario':
...
if __name__ == '__main__':    #Por si después quier reusar una función de acá (sin que se ejecute todo).      
    ...
    rta = input('Ingrese "c" para modo completo (generar todos los posibles valores) o nada para modo "diccionario" (ingrese "h" para ayuda)\nModo: ')
    if rta == 'c':
        ...
        inicio('diccionario') # En modo diccionario sólo evalúa las que coinciden con el diccionario.
